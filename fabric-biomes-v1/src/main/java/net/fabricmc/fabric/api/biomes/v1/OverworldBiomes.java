...
import net.fabricmc.fabric.impl.biomes.ClimateBiomeInjection;
...
public final class OverworldBiomes {
	...
	public static void addBiome(Biome biome, Climate climate, int weight) {
		Object2IntMap<Biome> weightMap = InternalBiomeData.WEIGHT_MAP.computeIfAbsent(climate, map -> new Object2IntOpenHashMap<>());
		weightMap.put(biome, weight + weightMap.computeIfAbsent(biome, a -> 0));

		for (int i = 0; i < weight; ++i) {
			InternalBiomeData.INJECTED_BIOMES.add(new ClimateBiomeInjection(biome, climate));
		}
	...
	public static void addBiomeVariant(Biome baseBiome, Biome variantBiome, int rarity) {
		InternalBiomeData.VARIANT_MAP.computeIfAbsent(baseBiome, biome -> new VariantAssociate()).addVariant(variantBiome, rarity);
	 ...
	 */
	public static void setRiverBiome(Biome baseBiome, Biome riverAssociate) {
		InternalBiomeData.RIVER_MAP.put(baseBiome, riverAssociate);

		if (riverAssociate != null) {
			InternalBiomeData.CUSTOM_BIOMES.add(riverAssociate);
		}
	}
	 ...
	 */
	public static int getWeight(Biome biome, Climate climate) {
		return InternalBiomeData.WEIGHT_MAP.containsKey(climate) ? 0 : InternalBiomeData.WEIGHT_MAP.get(climate).getOrDefault(biome, 0);
	}
